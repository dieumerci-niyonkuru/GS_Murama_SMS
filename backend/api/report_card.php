...
// Check authentication but allow parent access to own children
...
if ($user['role'] === 'Parent') {
    $email = $user['email'];
    $checkStmt = $db->prepare("SELECT student_id FROM students WHERE parent_email = ? AND student_id = ?");
...
// Class rank
$rankStmt = $db->prepare("SELECT s.student_id, AVG(er.score) as avg_score FROM students s LEFT JOIN exam_results er ON s.student_id = er.student_id WHERE s.class = ? GROUP BY s.id ORDER BY avg_score DESC");
